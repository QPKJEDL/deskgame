...
//    m_post_type = "login";
//    QByteArray postData;
//    postData.append("desk=vip13&password=123456");
//    m_accessManager->post(*m_request,postData);
//    qDebug() << "postdata: " << postData.data();
    ...
    if(way_big->now.m_now_line == 0 && way_big->now.m_now_row > 1){
        ...
        if(way_big->now.m_now_row > 2){
            ...
            if(way_big->now.m_now_row > 3){
                // 可以判断凹凸路
                if(if_order(4)){
                    style = way_aotu->labels[way_aotu->now.m_now_row][way_aotu->now.m_now_line].label->styleSheet() + QString("border-image: url(") + QString(":/result/red.png") + QString(");");
                    game = 7;
                }
                else{
                    style = way_aotu->labels[way_aotu->now.m_now_row][way_aotu->now.m_now_line].label->styleSheet() + QString("border-image: url(") + QString(":/result/blue.png") + QString(");");
                    game = 4;
                }
                // 移动到下一个凹凸路
