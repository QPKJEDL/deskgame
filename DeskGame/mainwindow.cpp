...
{
    m_request->setUrl(QUrl("http://" + URL + "/HeGuanNnGameRecord"));
    m_post_type = "record";
    QByteArray postData;
    //post
    m_accessManager->post(*m_request, postData);
...
void MainWindow::on_game_record(QJsonArray array)
{
    ...
    for(int i = array.count() - 1;i >= 0;i--){
        ...
        if(status == 2){
            ...
            QString show = JJtoNum(PaiXing);
            // 刷新显示
            result_list.at(quarter)->zhuang->setText(show);
            // 闲家1点数
            PaiXing = array.at(i)["x1num"].toString();
            show = JJtoNum(PaiXing);
            result_list.at(quarter)->one->setText(show);
            ...
            show = JJtoNum(PaiXing);
            result_list.at(quarter)->two->setText(show);
            ...
            show = JJtoNum(PaiXing);
            result_list.at(quarter)->three->setText(show);
...
{
    m_request->setUrl(QUrl("http://" + URL + "/HeGuanRoomCard"));
...
void MainWindow::on_room_card(QJsonArray zhuang, QJsonArray one, QJsonArray two, QJsonArray three)
{
