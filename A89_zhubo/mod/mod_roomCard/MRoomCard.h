...
private:
    void init_link();
    void apply_room_card(QJsonArray zhuang, QJsonArray one, QJsonArray two, QJsonArray three,QString zhuang_result,QString one_result,QString two_result,QString three_result);
    void responsed_roomcard(QNetworkReply *reply);
    void request_fapai(CARD card);
    void responsed_fapai(QNetworkReply *reply);
    void focus_lineedit();
    void append(string card, QString paixing);
